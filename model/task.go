...
// InsertAll は全てのTodoを格納します
// todoにIDを追加するという副作用があります
func (todos *Todos) InsertAll(tx *sqlx.Tx) error {
	// rangeで取得するとtodoがポインターで無いため、コピーされた値になっている
	for i := 0; i < len(*todos); i++ {
		r, err := (*todos)[i].Insert(tx)
		if err != nil {
			return err
		}
		(*todos)[i].ID, err = r.LastInsertId()
	}
	return nil
}
